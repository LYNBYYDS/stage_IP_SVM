...
void multiplier_signal(FILE * fp)
{
    ...
    for (i = 0; i <= 70; i++)
    {
        fprintf(fp,"signal multi_20bits_s%d : std_logic_vector(19 downto 0);\n", i);
    }
}
...
void connection_convertor(FILE * fp)
{
    int i, j;
    for (i = 0; i <= 70; i++)
    {
        fprintf(fp,"\n\tConvertor_Signed_13to20bits_%d : Convertor_Signed_13to20bits\n\tport map(\t", i);
        
        fprintf(fp,"input => multi_s%d,\n\t\t\t\t", i);
        fprintf(fp,"output => multi_20bits_s%d\n\t\t\t\t", i);
        fprintf(fp,");\n");
    }
}
...
{    
    ...
    fp = fopen("texttop.txt", "w");       // Open the file
